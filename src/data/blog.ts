export type ContentBlock = {
  type: "paragraph" | "heading" | "paragraph-1";
  value: string;
};

export type BlogType = {
  title: string;
  slug: string;
  img: string;
  content: ContentBlock[];
};


export const Blogs:BlogType[] = [
  {
    title: "Best Months to Cruise in Cyprus: A Year-Round Guide",
    img: "/blogs/blog1.jpg",
    slug: "best-months-to-cruise-in-cyprus-a-year-round-guide",
    content: [
      {
        type: "paragraph-1",
        value:
          "With its mild winters and hot summers, a visit to Cyprus is always a good idea, no matter the time of year. Here’s a detailed guide month by month to help you maximize your visit and enjoy Cyprus’s beautiful Mediterranean climate.",
      },
      { type: "heading", value: "January" },
      {
        type: "paragraph",
        value:
          "January in Cyprus is a bit colder, but temperatures rarely drop into single figures during the day. The first week is filled with festive events, and New Year’s Eve celebrations are vibrant across the island. Families and friends gather for meals, and the beaches are popular despite the cooler weather.",
      },
      { type: "heading", value: "February" },
      {
        type: "paragraph",
        value:
          "February sees shorter days and cooler temperatures, making it less crowded with tourists compared to warmer months. It’s a great time for skiing and snowboarding in the Troodos Mountains. While some areas are quieter, cities like Limassol, Larnaca, and Paphos offer affordable tourist activities.",
      },
      { type: "heading", value: "March" },
      {
        type: "paragraph",
        value:
          "March brings a colorful transformation to Cyprus with blooming flowers and shrubs. The Greek Orthodox Easter festivities include a ten-day carnival with feasts and parades, especially lively in Limassol. It’s a great time for outdoor activities like hikes, walks along the coast, quad biking, and exploring ancient monuments.",
      },
      { type: "heading", value: "April" },
      {
        type: "paragraph",
        value:
          "April is sunnier and warmer, perfect for sunbathing and engaging in water activities. It’s an ideal time for deep-sea diving to explore underwater spots. The Cyprus Film Days International Festival takes place in Nicosia, and water parks are open for fun-filled days. Cypriots observe the Lenten fast leading up to Easter celebrations.",
      },
      { type: "heading", value: "May" },
      {
        type: "paragraph",
        value:
          "May marks the start of new awakenings in Cyprus with various celebrations. The Anthestiria event celebrates spring and flowers, with flower displays in major cities. Events like accordion performances in Pera Oreinis offer cultural experiences. Daytime temperatures are high, perfect for outdoor activities.",
      },
      { type: "heading", value: "June" },
      {
        type: "paragraph",
        value:
          "June features the Festival of the Flood (Kataklysmos) and the Red Summer Festival in Cyprus. It’s a time for seaside fairs, music, dance, boat competitions, and watermelon season. The Cyprus International Film Festival is held around this time, offering opportunities to engage with filmmakers. The Shakespeare Festival at the ancient theatre of Kourion is a must-visit event.",
      },
      { type: "heading", value: "July" },
      {
        type: "paragraph",
        value:
          "July is a bustling month with visitors flocking to Cyprus for summer holidays. Wine and beer festivals are popular, along with various water sports activities. Ancient monuments and museums offer evening visits, while seaside areas host boat rides and foam parties. The Troodos mountains provide a cool escape from the summer heat.",
      },
      { type: "heading", value: "August" },
      {
        type: "paragraph",
        value:
          "August brings hot temperatures and endless beach parties in Cyprus. Harvest festivals in Lefkara and Omodos villages celebrate Mediterranean culture. The wine festival in Limassol honors the ancient god of vine and wine, Dionysus. Religious festivals like the Assumption of the Virgin Mary are observed in August.",
      },
      { type: "heading", value: "September" },
      {
        type: "paragraph",
        value:
          "September brings tranquility to the island as locals return to school and work. Tourists also start leaving, creating a peaceful atmosphere. Opera lovers can enjoy the Paphos Aphrodite Festival under the stars. Reduced hotel packages offer savings for late-season visitors.",
      },
      { type: "heading", value: "October" },
      {
        type: "paragraph",
        value:
          "October offers pleasant weather with warm sea breezes and warm sea waters. It’s a great time for swimming before winter sets in. Beach parties wind down as businesses prepare for winter, offering discounts for remaining tourists. It’s an ideal time to extend your summer holiday into November.",
      },
      { type: "heading", value: "November" },
      {
        type: "paragraph",
        value:
          "November marks the beginning of winter with more regular rainfall. It’s a good month for mountain walks, visiting ancient monuments, or engaging in windsurfing activities. The island becomes quieter as tourists depart, providing a serene atmosphere for outdoor activities.",
      },
      { type: "heading", value: "December" },
      {
        type: "paragraph",
        value:
          "December brings festive Christmas decorations and celebrations across Cyprus. Despite winter storms threatening the skies, Christmas markets offer last-minute shopping opportunities. Water sports enthusiasts can still enjoy activities in the sea, while clubs and restaurants organize holiday programs with exciting activities. The month culminates in Christmas Eve celebrations followed by various festivities leading into the New Year.",
      },
      {
        type: "paragraph",
        value:
          "Each month in Cyprus offers unique experiences and activities that cater to a variety of interests, making it a year-round destination worth exploring.",
      },
    ],
  },
  {
    title: "Approval Granted for New Marina and Cruise Ship Dock in Paphos",
    img: "/blogs/blogs1.jpg",
    slug: "approval-granted-for-new-marina-and-cruise-ship-dock-in-paphos",
    content: [
      {
        type: "paragraph-1",
        value:
          "The Cabinet has given approval for a new marina and cruise ship pier in Paphos, as announced by Tourism Minister Savvas Perdios. This development, which includes a hotel, marina, and a pier for luxury cruise ships, has been a goal of the Deputy Ministry for four years.",
      },
      {
        type: "paragraph",
        value:
          "The project had been inactive for 30 years until now. An expert study confirmed the technical feasibility of a mixed development, with the condition that the pier would only cater to cruise ships up to 220 meters long.",
      },
      {
        type: "paragraph",
        value:
          "This aligns with the Deputy Ministry’s aim to attract higher-income tourists. Integrated development proposals are favored by the Deputy Ministry for their potential to generate greater profits for the local economy, benefiting villages in the Paphos area and Polis Chrysochous.",
      },
      {
        type: "paragraph",
        value:
          "Local authorities and the Urban Planning Council have shown support for the project, contingent on the future investor providing environmental studies and a master plan when required. If all goes as planned, investors can expect final guidance by 2024.",
      },
      {
        type: "heading",
        value: "Hotel Upgrade Grant Scheme",
      },
      {
        type: "paragraph",
        value:
          "Additionally, the Cabinet has approved a grant scheme to upgrade hotels and rural tourist accommodations in mountainous or remote areas, with varying levels of funding allocated based on the location and type of accommodation.",
      },
      {
        type: "paragraph",
        value:
          "Accommodation in mountainous and remote areas will receive 75% of the upgrade cost, countryside accommodations will receive 65%, and underdeveloped tourist areas will receive 50% of the cost.",
      },
    ],
  },
  {
    title: "Best Months to Cruise in Cyprus: A Year-Round Guide",
    img: "/blogs/blogs4.jpg",
    slug: "best-months-to-cruise-in-cyprus-a-year-round-guide",
    content: [
      {
        type: "paragraph-1",
        value:
          "The Mediterranean is an attractive area comprising approximately 20 countries in Southern Europe and North Africa, bordered by the Mediterranean Sea with Cyprus being an island belonging to one of those countries.",
      },
      {
        type: "paragraph",
        value:
          "Its combination of cultural and religious sites, bustling cities, and peaceful islands makes it a perfect destination for a coastal cruise in the Mediterranean Sea with the most famous being a trip to the Blue Lagoon in Paphos and Protaras. Also, famous destinations are Coral Bay, Sea Caves, Akamas and Lara Bay.",
      },
      {
        type: "paragraph",
        value:
          "Cyprus also known as the “Island of Love” this region is a year-round destination with seasonal wine festivals and vibrant holiday markets. The ideal time to embark on a Mediterranean cruise depends on our yearly schedule whether you seek to enjoy the coastal sun or prefer to sail in popular destinations.",
      },
      {
        type: "paragraph",
        value:
          "Our cruises usually sail between the end of March until the middle of November. However, this is strongly dependent on weather conditions which might differ each year.",
      },
      {
        type: "paragraph",
        value:
          "Determining the best month for a Mediterranean cruise in Cyprus hinge on your vacation preferences. Factors such as weather, sea conditions, destinations, and pricing are considered to identify the most suitable travel dates for your needs.",
      },
      {
        type: "paragraph",
        value:
          "Our group of companies offers a unique opportunity of fulfilling above mentioned factors with multiple destination selection including Paphos, Latchi (Blue Lagoon) and Protaras, but also offering many boats/yachts/vessels of different styles, sizes and brands including Ocean Vision, Ocean Blue, Sea Star, Ocean Flyer, Mediterraneo III (catamaran), San Antonio II (traditional schooner), Mediterranean Diamond, Ocean Dream, Kurosivo IV, Jolly Roger (Pirate Ship), Ocean Destiny and Dolphin Twister (Adventure Jet Boat).",
      },
    ],
  },

  {
    title: "Our 11 Suggested Things to Do While in Paphos.",
    slug: "our-11-suggested-things-to-do-while-in-paphos",
    img: "/blogs/blogs3.jpg",
    content: [
      {
        type: "paragraph-1",
        value:
          "Anticipate an abundance of love and beauty in the birthplace of Aphrodite and indulge in the top activities in Paphos.",
      },
      {
        type: "paragraph",
        value:
          "Have you enjoyed some of our cruises and spent time at the sea? That’s amazing and we thank you for choosing us, we hope to see you back again. In this article you could discover the 11 best things to do in Paphos, a captivating harbor city in the southwest of Cyprus that is as enchanting as it appears in photos.",
      },
      {
        type: "paragraph",
        value:
          "Paphos exudes romance, not just because it’s Aphrodite’s birthplace but also due to its mosaic-adorned streets, picturesque sea coves at Coral Bay, and stunning beach sunsets. Here are the must-visit attractions when you explore this charming city.",
      },
      {
        type: "heading",
        value: "1. Aphrodite’s Rock",
      },
      {
        type: "paragraph",
        value:
          "Explore this massive sea rock, also known as the Rock of the Roman, a popular sight in Paphos with a fascinating mythological background.",
      },
      {
        type: "heading",
        value: "2. Tomb of Kings",
      },
      {
        type: "paragraph",
        value:
          "Delve into an underground maze of tomb chambers dating back to the fourth century BC within Kato Paphos Archaeological Park.",
      },
      {
        type: "heading",
        value: "3. Paphos Mosaics",
      },
      {
        type: "paragraph",
        value:
          "Admire the intricate mosaic collection at Kato Paphos Archaeology Park, showcasing colorful tiled floors from Roman villas.",
      },
      {
        type: "heading",
        value: "4. Paphos Castle",
      },
      {
        type: "paragraph",
        value:
          "Visit this centuries-old castle, a prominent landmark in Paphos that once served as a Byzantine fort and is now a venue for local festivals.",
      },
      {
        type: "heading",
        value: "5. Coral Bay",
      },
      {
        type: "paragraph",
        value:
          "Enjoy the soft white sand and crystal waters of this beautiful beach destination in Peyia, ideal for a relaxing day by the sea.",
      },
      {
        type: "heading",
        value: "6. Acropolis and Odeon",
      },
      {
        type: "paragraph",
        value:
          "Experience music events and theatre performances at this second-century theatre that continues to play a role in local arts.",
      },
      {
        type: "heading",
        value: "7. Adonis Baths",
      },
      {
        type: "paragraph",
        value:
          "Visit this peaceful waterfall with swimming areas and statues of Adonis and Aphrodite for a relaxing retreat beyond the beach.",
      },
      {
        type: "heading",
        value: "8. Paphos Old Town",
      },
      {
        type: "paragraph",
        value:
          "Take a leisurely stroll through the historic city center, dine outdoors, watch sunsets, and explore boutiques at Kennedy Square and Makariou Avenue.",
      },
      {
        type: "heading",
        value: "9. Chrysorrogiatissa Monastery",
      },
      {
        type: "paragraph",
        value:
          "Discover this twelfth-century monastery nestled in the Cypriot mountains near Paphos, known for its tranquil setting and vintage wine production.",
      },
      {
        type: "heading",
        value: "10. Saint Paul’s Pillar",
      },
      {
        type: "paragraph",
        value:
          "Visit the pillar where Saint Paul suffered lashings for preaching Christianity during his visit to Paphos in 45 AD, next to the impressive Agia Kyriaki Chrysopolitissa church with mosaic-tiled floors.",
      },
      {
        type: "heading",
        value: "11. Saint Neophytos Monastery",
      },
      {
        type: "paragraph",
        value:
          "The Agios Neophytos monastery, situated approximately 9km north of Pafos (Paphos), was established by Neophytos, a Cypriot recluse and writer, in the latter part of the 12th century. It was originally located in a secluded spot at the top of a picturesque valley.",
      },
    ],
  },
];
